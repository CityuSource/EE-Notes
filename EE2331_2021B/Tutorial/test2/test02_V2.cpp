...
{
	return ((op1 == '+' || op1 == '-' || op1 == '*' || op1 == '/') && (op2 == '+' || op2 == '-')) ? true : false;
}
...
string infix2postfix(string infix)
{
	...
	for (auto &subchar : infix)
	{
		...
		else if (subchar == '+' || subchar == '-' || subchar == '*' || subchar == '/')
		{
			for (; !stack.empty() && hasHigherPrecedence(stack.top(), subchar); stack.pop())
				postfix += stack.top();
			stack.push(subchar);
		}
	}
