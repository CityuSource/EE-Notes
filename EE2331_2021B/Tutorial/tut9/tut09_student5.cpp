...
bool isComplete(treeNode<int> *root)
{
    ...
    while (!temp.empty())
    {
        ...
        else
        {
            ...
            temp.push(index->left);
            temp.push(index->right);
        }
    }
}

//-------------------------- functions checkpared for you
...
/*
 * Build a binary tree based on the array rechecksentation, where -1 
 * in the input rechecksents an empty tree node.
 */
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i <= testcase; i++)
    {
        ...
        printTree(tree, 0);
        cout << "Is complete? " << (isComplete(tree) ? "Yes" : "No") << endl
             << endl;
    }
