...
bool isComplete(treeNode<int> *root)
{
    ...
    return (!root) ? 1 : ((!root->left && !root->right) ? 1 : ((root->left && root->left->left && !root->left->right) ? 0 : ((root->left && !root->right) ? 1 : ((!root->left && root->right) ? 0 : ((root->left && root->right) ? (isComplete(root->left) && isComplete(root->right)) : 0)))));
}
