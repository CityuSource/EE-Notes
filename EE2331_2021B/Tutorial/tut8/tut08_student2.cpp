...
{
    int the_END = 1; //this variable is for each move
  
    for (int m = 0; m < 8; m++)
    {
        if (chessboard[x + xMove[m]][y + yMove[m]] == -1 && x + xMove[m] >= 0 && x + xMove[m] < N && y + yMove[m] >= 0 && y + yMove[m] < N) //this is to check if the next move is valid or not
        {
            chessboard[x + xMove[m]][y + yMove[m]] = step;
            the_END = 0;

            solve(x + xMove[m], y + yMove[m], step + 1, chessboard, count);
            chessboard[x + xMove[m]][y + yMove[m]] = -1;
        }
    }

    if (the_END)
    {
        bool if_occupied = true; // this is to check if all the positions are occupied or not
        for (int m = 0; m < N; m++)
        {
            for (int n = 0; n < N; n++)
            {
