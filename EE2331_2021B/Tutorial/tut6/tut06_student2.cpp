...
node<int>* reverseList(node<int>* head) {
	...
	while (head != NULL)
	{
		stack.push(head);  //pushing the node to back of the stack
		head = head->next; //moving head forward
	}

	node<int> newHead = { NULL, NULL }; //newHead of the list
	node<int>* current = &newHead;        //temporary node to make the list
	while (stack.empty() == false)
	{
		current->next = new node<int>({ stack.top()->data, NULL }); //creating the next node of the list
		current = current->next;                 //moving cur forward
		stack.pop();                     //poping the last element from stack
	}
