...
{
	return osObject << myDate.year << ((myDate.month < 10) ? "-0" : "-") << myDate.month << ((myDate.day < 10) ? "-0" : "-") << myDate.day;
}
...
{
	auto objA = (invoice *)a, objB = (invoice *)b;
	return (objA->date.year == objB->date.year) ? (objA->date.month == objB->date.month) ? objB->date.day - objA->date.day : objB->date.month - objA->date.month : objB->date.year - objA->date.year;
}
...
{
	auto objA = (invoice *)a, objB = (invoice *)b;
	return (objA->amount != objB->amount) ? objA->amount - objB->amount : -compareInvoiceDate(objA, objB);
}
