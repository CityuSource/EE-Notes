...
{
	osObject << myDate.year << "-";

	if (myDate.month < 10)
		osObject << "0";
	osObject << myDate.month << "-";
...
int compareInvoiceDate(const void* a, const void* b) {
	...
	invoice* d1 = (invoice*)a;
	invoice* d2 = (invoice*)b;

	if (d1->date.year != d2->date.year)
		return d2->date.year - d1->date.year;

	if (d1->date.month != d2->date.month)
		return d2->date.month - d1->date.month;

	return d2->date.day - d1->date.day;
}
...
int compareInvoiceAmountDate(const void* a, const void* b) {
	...
	return -1 * compareInvoiceDate(a, b);
}
