...
treeNode<char>* reconstructTreeUsingInorderPostorder(char inorder[], int inLow, int inHigh, char postorder[], int postLow, int postHigh) {
	...
	if(inLow > inHigh || postLow > postHigh)
        ...
        while(node->info != inorder[SI])
                SI++;
        node->left  = reconstructTreeUsingInorderPostorder(inorder,inLow,SI-1,postorder,postLow,postLow+SI-inLow-1);
        node->right = reconstructTreeUsingInorderPostorder(inorder,SI+1,inHigh,postorder,postLow+SI-inLow,postHigh-1);
