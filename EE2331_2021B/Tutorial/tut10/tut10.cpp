...
treeNode<char> *reconstructTreeUsingInorderPostorder(char inorder[], int inLow, int inHigh, char postorder[], int postLow, int postHigh)
{
    ...
    auto *root = new treeNode<char>{postorder[postHigh], nullptr, nullptr};

    if (inLow < inHigh)
    {
        int index;
        for (index = inLow; index <= inHigh && root->info != inorder[index]; index++)
            ; // (index - inLow) -> length of the postorder sequence
        root->left = reconstructTreeUsingInorderPostorder(inorder, inLow, index - 1, postorder, postLow, postLow + index - inLow - 1);
        root->right = reconstructTreeUsingInorderPostorder(inorder, index + 1, inHigh, postorder, postLow + index - inLow, postHigh - 1);
    }
