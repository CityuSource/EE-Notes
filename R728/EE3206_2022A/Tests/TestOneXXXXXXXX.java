...
public class TestOneXXXXXXXX {

    public static void main(String[] args) {

        // Load the CSV data file. Each row is converted to an instance of Record and saved to a list.
        String filepath = TestOneXXXXXXXX.class.getResource("number-of-internet-users-by-country.csv").getFile();
        ...
        System.out.println("The country of most no. of internet users in 2000: " + countryOfMostUsersInYear(records, 2000));
        System.out.println("The country of most no. of internet users in 2015: " + countryOfMostUsersInYear(records, 2015));
    ...
    /**
     * This method finds the country having most number of internet users in the specific year.
     *
     * @param records a list of records from the CSV file
     * @param year a specific year
     * @return the name of the country having most number of internet users in the year
     */
    ...
    /**
     * This method computes the total number of internet users in the world for each year. A world record can
     * be constructed using the second constructor in the Record class which presets "World" as the country
     * name and "OWID_WRL" as the country code. The list of world records should be sorted by year in descending
     * order.
     *
     * @param records a list of records from the CSV file
     * @return a list of records that represent the total number of internet users in the world for each year, sorted by year descendingly
     */
    ...
    /**
     * This method computes the annual growth of the number of internet users of the specified country.
     * It returns the top three years with highest growth in descending order.
     * 
     *      annual growth of current year = (no. of users in current year - no. of user in preceding year) / no. of user in preceding year
     * 
     * An entry in the returned list is a key-value pair that associates the year with the growth. To create it, you can use 
     * Map.entry() (for JDK 9 and after) or the helper method entry() defined at the end of this class (for JDK 8).
     * 
     * @param records a list of records from the CSV file
     * @param code a specific country code
     * @return a list of entries of the top three years with highest annual growth, sorted by growth in descending order
     */
    static List<Entry<Integer, Double>> yearsOftop3Growth(List<Record> records, String code) {
        ...
        for (int i = 1; i < filtered.size(); i++) {
            if (filtered.get(i - 1).getUsers() > 0) {
                double growth = (double) (filtered.get(i).getUsers() - filtered.get(i - 1).getUsers()) / filtered.get(i - 1).getUsers();
...
class Record {
    ...
    /**
     * Construct a record for the number of internet users in the world, not a particular country. 
     * Country name is set to "World" by default. 
     * Country code is set to "OWID_WRL" by default.
     */
