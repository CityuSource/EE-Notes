...
treeNode<char>* reconstructTreeUsingInorderPostorder(char inorder[], int inLow, int inHigh, char postorder[], int postLow, int postHigh) {
    if (inLow > inHigh)
        return NULL;
    treeNode<char>* node = (treeNode<char>*)malloc(sizeof(treeNode<char>));
    node->info = postorder[postHigh];
    node->left = NULL;
    node->right = NULL;
    int shift;
    for (int i = 0; i <= inHigh - inLow; i++) {
        if (inorder[i + inLow] == node->info) {
            shift = i;
            break;
        }
    }
    node->right = reconstructTreeUsingInorderPostorder(inorder, inLow + shift + 1, inHigh, postorder, postLow + shift, postHigh - 1);
    node->left = reconstructTreeUsingInorderPostorder(inorder, inLow, inLow + shift - 1, postorder, postLow, postLow + shift - 1);
    return node;
}
