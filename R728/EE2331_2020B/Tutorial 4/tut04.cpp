...
node *buildReverseList(char *number)
{
	...
	head->data = number[0] - 48;
	head->next = NULL;

	for (int i = 1; i < strlen(number); i++)
	{
		node *temp1 = new node; //Put the new number as 'head' and then make it point to a new node with its old values.
		temp1->data = number[i] - 48;
		temp1->next = head;
		head = temp1;
	}
...
{
	node *head = new node;
	int sum = num1->data + num2->data; //First digit's addition
	head->data = sum % 10;
	...
	for (int a, b; num1 || num2; num1 ? num1 = num1->next : NULL, // Check if either value is null or not then continues the loop
								 num2 ? num2 = num2->next : NULL)
	{
		num1 ? a = num1->data : a = 0; // Checks which one is null or not
		num2 ? b = num2->data : b = 0;
		sum = a + b + carry;
		carry = sum / 10;

		temp1->next = new node; // Adds the data together then put it in a new node on the next position
		temp1 = temp1->next;
		temp1->data = sum % 10;
		temp1->next = NULL;
	}

	if (carry == 1) // To catch any leftover carry bits
	{
		temp1->next = new node;
		temp1 = temp1->next;
		temp1->data = carry;
