...
*
*/
bool isBST(treeNode<int> *root, treeNode<int> *min, treeNode<int> *max) { return !root ? true : (((min && root->key <= min->key) || (max && root->key >= max->key)) ? false : (isBST(root->left, min, root) && isBST(root->right, root, max))); }
 ...
 */
bool isBST(treeNode<int> *root, treeNode<int> *&prev) { return !root ? true : (!isBST(root->left, prev) ? false : ((prev && root->key <= prev->key) ? false : (prev = root, isBST(root->right, prev)))); }
