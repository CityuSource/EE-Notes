...
u_result RSlidarDriverSerialImpl::_waitScanData(RSLIDAR_SIGNAL_DISTANCE_UNIT_T * nodebuffer, size_t & count, _u32 timeout)
{
	...
	//while (waitTime <= timeout && recvNodeCount < count)
	{
		...
		startAngle = (recvBuffer[11] << 8) | recvBuffer[12];
		endAngle = (recvBuffer[13] << 8) | recvBuffer[14];
		...
		if (crcCheckNum == calcchecksum)
		{
			...
			if ((angleRange >= 0) && (angleRange <= 500))  //find original point
			{
				flagOfNewNodes = true;
				recvNodeCount = 0;
				sampleNumberCount = 0;
				angleStep = (_u16)(endAngle - startAngle) / (sampleNumber - 1);
			}
			...
			if (flagOfNewNodes)
			{
				...
				tempoffset = (recvBuffer[9] << 8) | recvBuffer[10];
				if (tempoffset >> 15)
				{
					tempoffset = ~tempoffset + 1;
					angleoffset = (float)tempoffset * -1;
				}
				else
				{
					angleoffset = (float)tempoffset;
				}
				...
				{
					(nodebuffer + sampleNumberCount)->speed = speed;
					(nodebuffer + sampleNumberCount)->angle = startAngle + i * angleStep;
					(nodebuffer + sampleNumberCount)->signalValue = recvBuffer[15 + i * 3];
					(nodebuffer + sampleNumberCount)->angleoffset = angleoffset / 100;
					(nodebuffer + sampleNumberCount)->distanceValue = (recvBuffer[16 + i * 3] << 8) | recvBuffer[17 + i * 3];
#ifdef USE_CONSOLE
