...
struct process_t
{
	...
	void log_w(int time) { this->working.push_back(time); };
};

struct queue_list
{
	vector<process_t> input_queue;
	vector<process_t> ready_queue;
	vector<process_t> block_queue;
	vector<process_t> finish_queue;

	int cur_process_id;
	int prev_process_id;
	queue_list() : cur_process_id(-1), prev_process_id(-1) {}
	queue_list(vector<process_t> queue) : input_queue(queue), cur_process_id(-1), prev_process_id(-1) {}
};
	...
	for (int cur_tick = 0; cur_tick < MAX_LOOP; cur_tick++)
	{
		...
		for (int i = 0; i < processes.size(); i++)
		{
			...
			{ // process arrives at current tick
				list.ready_queue.push_back(processes[i]);
			}
		}

		// I/O device scheduling
		if (!list.block_queue.empty())
		{
			process_t &cur_io_process = list.block_queue.front(); // always provide service to the first process in block queue
			...
			{ // I/O service is completed
				...
				move_process_from(list.block_queue, list.ready_queue);
			}
			cur_io_process.cur_service_tick++; // increment the num of ticks that have been spent on current service
		}
		// CPU scheduling
		if (list.ready_queue.empty())
		{							   // no process for scheduling
			list.prev_process_id = -1; // reset the previous dispatched process ID to empty
		}
		else
		{
			process_t &cur_process = list.ready_queue.front(); // always dispatch the first process in ready queue
			list.cur_process_id = cur_process.process_id;

			if (list.cur_process_id != list.prev_process_id)
			{ // store the tick when current process is dispatched
				cur_process.log_w(cur_tick);
			}
			...
			{ // current service is completed
				...
				{ // the whole process is completed
					cur_process.log_w(cur_tick + 1);
					move_process_from(list.ready_queue, list.finish_queue); // remove current process from ready queue
				}
				else if (cur_process.cur_service.type == "D")
				{ // next service is disk I/O, block current process
					cur_process.log_w(cur_tick + 1);
					move_process_from(list.ready_queue, list.block_queue);
				}
			}
			list.prev_process_id = list.cur_process_id; // log the previous dispatched process ID
		}
		if (list.finish_queue.size() == processes.size())
		{ // all process completed
			break;
		}
	}
	write_file(list.finish_queue, output_path); // write output
	return 1;
}
